...
- **v1.3 — reach (started):** [x] directory browser — `mdv` with no argument or a directory scans recursively for `*.md`/`*.markdown` (skipping dot dirs and `node_modules`/`vendor`/`target`, capped at 5000), filters with the TOC's fuzzy ranker, opens on `Enter`; quitting a document returns to the list, `Ctrl+C` leaves. [x] multi-file buffers — several paths reuse that list rather than inventing a second switcher UI. [x] remote fetch (`internal/fetch`) — http/https with a 20s timeout and 8 MiB cap, `github.com/owner/repo` resolving to the README and `blob` links to raw; one remote document at a time, relative links unresolved as with stdin. [ ] Mermaid. Sixel/iTerm2: see §4.
