...
- **v1.3 — reach (started):** [x] directory browser — `mdv` with no argument or a directory scans recursively for `*.md`/`*.markdown` (skipping dot dirs and `node_modules`/`vendor`/`target`, capped at 5000), filters with the TOC's fuzzy ranker, opens on `Enter`; quitting a document returns to the list, `Ctrl+C` leaves. [ ] multi-file buffers, remote fetch, Sixel/iTerm2, Mermaid.
