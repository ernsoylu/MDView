...
// go-latex v0.3.0 panics (not errors) on TeX outside its subset — notably
// superscripts and subscripts are not typeset yet — so recover turns any
// panic into an error and the caller falls back to raw TeX.
func renderMath(expr string) (m image.Image, fgKey string, err error) {
	defer func() {
		if r := recover(); r != nil {
			m, fgKey, err = nil, "", fmt.Errorf("mtex: %v", r)
		}
	}()
	...
	return img.AlphaFromLuminance(src, fg), fgKey, nil
}
